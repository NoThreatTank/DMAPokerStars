...
void CreateGUI()
{
	...
	auto form = std::make_shared<Form>(100, 100.0f, 300, 200, 2, 30, LIT(L"DMA PokerStars"), false);
	{
		auto tabcontroller = std::make_shared<TabController>();
		form->Push(tabcontroller);

		auto playeresptab = std::make_shared<Tab>(LIT(L"Cards ESP"), 5, 5, &SelectedTab, 0, 20);
		{
			auto name = std::make_shared<Toggle>(100, 5, LIT(L"Player cards"), &Configs.Survivor.Name);
			...
			auto distance = std::make_shared<Toggle>(100, 25, LIT(L"Win chance"), &Configs.Survivor.Distance);
